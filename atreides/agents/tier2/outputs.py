...
``atreides.agents.tier2`` produces. It depends only on the contracts
substrate (``atreides.contracts``) and on its own enumerations.
...
# DSOR lineage hash format (see atreides.contracts.dsor_stub) so that
...
class EligibilityVerification(BaseModel):
    ...
    once; the agent (in :mod:`atreides.agents.tier2.eligibility`)
